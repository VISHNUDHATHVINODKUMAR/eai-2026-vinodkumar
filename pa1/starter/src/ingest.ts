 ...
 *
 * The file is CP1257. Node will happily read it as UTF-8 and give you
 ...
 *
 * Convert to an ISO-8601 calendar date. Do not route this through
 ...
 *
 * Return a decimal STRING. The moment this value becomes a float you
 ...
 *
 * This file is UTF-8, unlike the order file. No field contains a
...
export function ingest(options: IngestOptions): Report {
  ...
  // Split into lines; tolerate LF or CRLF, and a trailing newline (or not)
  // on the last line.
  ...
  lines.forEach((line, index) => {
    ...
    const [orderIdStart, orderIdEnd] = ORDER_LAYOUT.orderId;
    const [customerIdStart, customerIdEnd] = ORDER_LAYOUT.customerId;
    const [nameStart, nameEnd] = ORDER_LAYOUT.customerName;
    const [dateStart, dateEnd] = ORDER_LAYOUT.orderDate;
    const [amountStart, amountEnd] = ORDER_LAYOUT.amount;
    const [currencyStart, currencyEnd] = ORDER_LAYOUT.currency;

    orders.push({
      orderId: line.slice(orderIdStart, orderIdEnd).trim(),
      customerId: line.slice(customerIdStart, customerIdEnd).trim(),
      customerName: line.slice(nameStart, nameEnd).trim(),
      orderDate: toIsoDate(line.slice(dateStart, dateEnd).trim()),
      amount: toDecimalString(line.slice(amountStart, amountEnd)),
      currency: line.slice(currencyStart, currencyEnd).trim(),
    });
  });
  ...
  const orderedCustomerIds = new Set(orders.map((o) => o.customerId));
  const unmatchedCustomers = [...customers.keys()].filter(
    (id) => !orderedCustomerIds.has(id),
  );
